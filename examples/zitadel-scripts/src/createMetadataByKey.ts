...
async function main(): Promise<void> {
  ...
  console.log('Creating metadata by key...')
  ...
  const value = 'test metadata value'

  const randomKey = Array.from({ length: 6 }, () => Math.random().toString(36)[2]).join('')

  const metadata = await zitadelClient.createMetadataByKey(
    {
